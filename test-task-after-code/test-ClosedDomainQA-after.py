...
with open("./data/task-data/test-ClosedDomainQA.jsonl", "r+", encoding="utf8") as f:
    ...
    for data in jsonlines.Reader(f):
        if mode =="Batch-Sequential":
            ...
            result = open(f"./test-result/test-ClosedDomainQA/result-ClosedDomainQA-{mode}-{method}-{batch_size}*{str(edit_time)[-1]}.txt", "a", encoding="utf8")
        else:
            result = open(f"./test-result/test-ClosedDomainQA/result-ClosedDomainQA-{mode}-{method}{sample_total}.txt", "a", encoding="utf8")
        ...
        if ('yes' in predict.lower()) or ('no' in predict.lower()):
            if ('yes' in predict.lower()) and (answers):
                TP = TP + 1
            elif ('no' in predict.lower()) and (not(answers)):
                FN = FN + 1
            elif ('no' in predict.lower()) and (answers):
                TN = TN + 1
            elif ('yes' in predict.lower()) and (not(answers)):
                FP = FP + 1
        else:
            others = others + 1
        result.close()
